...
const { Vehiculo, Usuario, RegistroServicioVehiculo, CotizacionServicioVehiculo } = require("../Models")
...
const registrarVehiculo = async (req, res) => {
    ...
    const cliente = await Usuario.findByPk(id_rol);
    //validar que el cliente tenga el estado activo
    if (!cliente || cliente.estado !== 'ACTIVO') {
        return res.status(400).json({ error: 'Cliente no válido o no está activo' });
    }
    if (!cliente || cliente.rol !== 3) { // 3 es el id del rol cliente
