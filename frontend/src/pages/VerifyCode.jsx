...
export default function VerifyCode() {
  ...
  const handlePaste = (e) => {
    ...
    setDigits(next);
    // Enfocar el último que tenga valor
  ...
  const onSubmit = async (e) => {
    ...
    try {
       const token = localStorage.getItem('token');
       const { data } =await api.post('/personas/autenticar-codigo-verificacion', { token:token, codigo_verificacion:code });
       console.log(data);
  ...
  const resendCode = async () => {
    try {
      await api.post('/personas/resend-code'); // ajusta si es distinto
    ...
    <div className="login-bg d-flex align-items-center justify-content-center vh-100">
      <div className="p-4 login-card" style={{ width: '100%', maxWidth: 420 }}>
        ...
        <div className="text-center mt-3">
          <button className="btn btn-link p-0" onClick={resendCode}>
            Reenviar código
          </button>
        </div>
      </div>
